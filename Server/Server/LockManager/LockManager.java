...
public class LockManager
{
	...
	public boolean UnlockAll(int xid)
	{
		...
		synchronized(lockTable) {
			Vector vect = lockTable.elements(lockQuery);
					...
					for (int j = 0; j < waitSize; j++)
					{
						...
						if (waitLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
						{
							if (j == 0)
							{
								...
								Vector vect1 = lockTable.elements(dataLockObject);
	...
	{
		Vector vect = lockTable.elements(dataLockObject);
		int size = vect.size();
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (!Objects.equals(l_dataLockObject.getDataName(), dataLockObject.getDataName())) {
			    continue;
            }
	...
	private void WaitLock(DataLockObject dataLockObject) throws DeadlockException
	{
		...
		synchronized (stampTable) {
			Vector vect = stampTable.elements(timeObject);
			if (vect.size() == 0)
			{
